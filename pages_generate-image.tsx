...
const texts = {
  ...
  en: {
    ...
    back: "Back to Chat",
    credits: "Credits:",
    cost: "Cost: 10 credits per image",
    error: {
      noCredits: "Not enough credits! Need 10 credits.",
  ...
  jp: {
    ...
    back: "チャットに戻る",
    credits: "クレジット：",
    cost: "コスト：画像1枚につき10クレジット",
    error: {
      noCredits: "クレジットが足りません！10クレジットが必要です。",
      failed: "画像の生成に失敗しました。もう一度お試しください。",
    }
  }
};
...
const GenerateImagePage: React.FC<Props> = ({ onBack, email, credits, onCreditsUpdate }) => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.message || texts[lang].error.failed);
      }
